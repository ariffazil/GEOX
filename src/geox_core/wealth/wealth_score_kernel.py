...
async def geox_to_wealth_score(
    ...
    # 1. THE HARNESS AUDIT
    audit_res = engine.audit(tool_name="geox_to_wealth_score", primary=primary_data, flags=flags, parent_hash=parent_chain_hash)
    ...
    outcomes = substrate_evidence.get(
        "scenarios", [{"probability": 0.3, "outcome": primary_data["hcpv"] * 50}, {"probability": 0.7, "outcome": -1000000}]
    )
