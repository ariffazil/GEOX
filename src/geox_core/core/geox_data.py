...
def generate_well_curve(
    ...
    for i, d in enumerate(md):
        ...
        gr[i] = rng.normal(layer.gr_min + (layer.gr_max - layer.gr_min) * (1 - layer.vsh_mean), noise_level * 25)

        vsh_val = np.clip((gr[i] - layer.gr_min) / max(layer.gr_max - layer.gr_min, 1e-6), 0, 1)
        vsh[i] = vsh_val
