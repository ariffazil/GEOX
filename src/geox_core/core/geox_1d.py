...
def compute_sw_indonesian(
    rt: np.ndarray, rn: np.ndarray, phi: np.ndarray, vsh: np.ndarray, rw: float = 0.03, a: float = 1.0, m: float = 2.0
) -> np.ndarray:
    """
    ...
    sw = (alpha * (np.sqrt(rt_safe) + np.sqrt(rsh_est)) / (np.sqrt(rt_safe) * (1 - vsh_safe))) ** (2 / (1 + vsh_safe))
    return np.clip(sw, 0, 1)
