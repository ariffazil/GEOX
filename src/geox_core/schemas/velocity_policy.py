...
class VelocityACRiskPolicy(BaseModel):
    ...
    # Thresholds
    max_acceptable_ac_risk: float = Field(
        default=0.60, description="Above this risk, the volume is rejected as a structural proxy."
    )
    base_irreducible_risk: float = Field(
        default=0.10, description="The baseline epistemic uncertainty inherent in any velocity model."
    )

    # Penalty weights for missing QC gates
    qc_penalty_map: dict[VelocityQCGate, float] = Field(
    ...
    failure_mode_penalty_map: dict[VelocityFailureMode, float] = Field(
        default_factory=lambda: {
