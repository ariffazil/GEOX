...
class VelocityStructuralClaim(BaseModel):
    ...
    epistemic_rung: str = Field(
        default="EARTHMODEL", description="Velocity cubes for structure are always Rung 5 models, never facts."
    )
    ...
    evidence_handles: list[str] = Field(
        default_factory=list, description="URIs pointing to QC artifacts (e.g., geox://qc/welltie/123)."
    )
    ...
    uncertainty_band: str = Field(
        default="P10-P90 Not Calculated", description="Uncertainty spread across realizations (e.g., '±50m at 3km depth')."
    )
    ac_risk: float = Field(
        ..., ge=0.0, le=1.0, description="Calculated Anomalous Contrast Risk. >0.70 means highly unsafe for structural decisions."
    )
    is_structural_proxy_valid: bool = Field(
        ..., description="Final verdict on whether this volume can be used for structural framework."
    )
