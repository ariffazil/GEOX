...
class EarthMemoryStore:
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                (
                    ...
                    asset_id,
                    "earth",
                    payload.get("truth_class", "interpretation"),
                    "draft",
                    json.dumps(payload),
                    timestamp,
                ),
    ...
    def validate_claim(self, record_id: str) -> bool:
        ...
        with self._connect() as conn:
            ...
            conn.execute(
                "UPDATE earth_memory SET approval_state = 'validated', payload = ? WHERE id = ?", (json.dumps(payload), record_id)
            )
        return True
    ...
    def review_interpretation(self, record_id: str, reviewer: str) -> bool:
        ...
        with self._connect() as conn:
            ...
            conn.execute(
                "UPDATE earth_memory SET approval_state = 'reviewed', payload = ? WHERE id = ?", (json.dumps(payload), record_id)
            )
        return True
    ...
    def seal_decision(self, record_id: str) -> dict:
        ...
        with self._connect() as conn:
            ...
            conn.execute(
                "UPDATE earth_memory SET approval_state = 'sealed', payload = ?, vault_receipt = ? WHERE id = ?",
                (json.dumps(payload), receipt["hash"], record_id),
            )
            return receipt
    ...
    def store(self, asset_id: str, eval_type: str, payload: dict[str, Any], amanah_locked: bool = False) -> Any:
        ...
        if not amanah_locked:
            return MockStoreResult(
                False,
                None,
                ["F11", "authorization required: amanah_locked=False"],
                _receipt("geox_memory_store_asset", {}, "HOLD"),
            )
