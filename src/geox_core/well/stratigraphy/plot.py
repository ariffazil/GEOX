...
WELL_COLORS = [
    "#C5D9F1", "#E2EFDA", "#FCE4D6", "#FFF2CC",
    "#FFE0E0", "#EAD1DC", "#D9D2E9", "#CFE2F3",
]
IVL_COLORS = plt.cm.Dark2.colors


def _plot_gr_conventional(ax, depth, gr, gr_min, gr_max, gr_cut,
                           sand_color, shale_color, curve_color, alpha=0.70):
    ...
    ax.fill_betweenx(dd, gg, gr_max, where=(gg <= gr_cut),
                     color=sand_color, alpha=alpha, linewidth=0, zorder=2)
    ax.fill_betweenx(dd, gg, gr_max, where=(gg > gr_cut),
                     color=shale_color, alpha=alpha, linewidth=0, zorder=2)
...
def _draw_motif_schematic(ax, motif, color, top_d, base_d,
                          x0=0.02, x1=0.40, alpha_override=None):
    ...
    if motif == "Fining Upward":
        ...
        ax.fill_betweenx(y, x0, xr, color=color,
                         alpha=max(0.08, alp - 0.15), zorder=4, lw=0)
        ...
        ax.fill_betweenx([top_d, base_d], x0, x0 + span * 0.50,
                         facecolor='none', edgecolor=color, hatch='////',
                         lw=0.6, zorder=4)
        ax.fill_betweenx([top_d, base_d], x0, x0 + span * 0.50,
                         color=color, alpha=min(alp * 0.26, 0.18), zorder=3, lw=0)
...
def generate_well_panel(
    ...
    for ivl in intervals:
        age = _parse_zone_age(str(ivl.zone if hasattr(ivl, 'zone') else ivl.get("Z", "")),
                              nn_ages)
        if age:
    ...
    if len(d_pts) >= 2:
        ...
        try:
            age_fn = interp1d(da[si], aa[si], bounds_error=False,
                              fill_value=(aa[si][0], aa[si][-1]))
        except Exception:
            pass
    ...
    if age_fn is not None:
        ...
        sl_v = np.array([
            float(np.interp(float(age_fn(dd)), _haq_ogg[:, 0], _haq_ogg[:, 1]))
            for dd in sl_d
        ])
    ...
    gs = gridspec.GridSpec(1, 6, figure=fig,
                           ...
                           wspace=0.03, left=0.065, right=0.985,
    ...
    if dep_f is not None and gr_f is not None:
        _plot_gr_conventional(ax_gr, dep_f, gr_f,
                              config.gr_min_api, config.gr_max_api, config.gr_cut_api,
                              config.sand_color, config.shale_color, config.curve_color,
                              alpha=0.58)
    ...
    for ii, ivl in enumerate(intervals):
        ...
        if d_min <= mid <= d_max:
            ...
            ax_gr.text(3, mid, f"{zn}\n{a_s}", fontsize=6, va="center",
                       ha="left", zorder=6, color=ic, fontweight="bold",
                       bbox=dict(boxstyle="round,pad=0.15", fc="white",
                                 ec=ic, alpha=0.88, lw=0.9))

    ax_gr.set_title(
        f"GR Log  (API)\n0 ← Sand  |  {int(config.gr_cut_api)} cut  |  Shale → {int(config.gr_max_api)}",
        fontsize=10, fontweight="bold", pad=6)
    ax_gr.legend(handles=[
        mpatches.Patch(color=config.sand_color,
                       label=f"Sand  (<{int(config.gr_cut_api)} API)"),
        mpatches.Patch(color=config.shale_color,
                       label=f"Shale (>{int(config.gr_cut_api)} API)"),
    ], loc="lower right", fontsize=7, framealpha=0.95, handlelength=1.2)
    ...
    if sensing_bins:
        for b in sensing_bins:
            ...
            if b.get("N", 0) < 3:
                ax_lt.axhspan(tc, bc, facecolor="none",
                              edgecolor="white", hatch="xxx", lw=0, alpha=0.55)

    ax_lt.legend(handles=[
        mpatches.Patch(color=c, label=k) for k, c in lit_colors.items()
    ], loc="lower center", fontsize=4.5, framealpha=0.95,
       bbox_to_anchor=(0.5, -0.01), ncol=1, handlelength=1.0)

    # Track 2: Geological Package + Motif + Seq Strat
    ax_pkg.set_xlim(0, 1)
    ax_pkg.set_xticks([])
    ax_pkg.tick_params(axis="y", left=False, labelleft=False)
    ax_pkg.set_title("Geological Package\nMotif Schematic  |  Systems Tract",
                     fontsize=10, fontweight="bold", pad=6)
    ...
    for pkg in packages:
        ...
        ax_pkg.axhspan(top_c, base_c, xmin=0.43, xmax=1.0,
                       color=tc, alpha=0.38, lw=0, zorder=2)
        ...
        if motif == "Heterolithic" and em:
            txt = (f"~{SHORT_MOTIF.get(em, em[:6])}\n"
                   f"[extrap·{pkg.get('EXTRAP_CONF','?')[:1]}]\n"
                   f"≈HET\n{int(pkg['TOP'])}–{int(pkg['BASE'])} m")
        else:
            txt = (f"{tract}\n{SHORT_MOTIF.get(motif, motif[:8])}\n"
                   f"{int(pkg['TOP'])}–{int(pkg['BASE'])} m\n"
                   f"{pkg.get('GR_MEAN','?'):.0f} API" if pkg.get("GR_MEAN") else
                   f"{tract}\n{SHORT_MOTIF.get(motif, motif[:8])}\n"
                   f"{int(pkg['TOP'])}–{int(pkg['BASE'])} m")

        if thick_d >= 20:
            ax_pkg.text(0.71, mid, txt, ha="center", va="center",
    ...
    ax_pkg.legend(
        handles=[mpatches.Patch(color=TRACT_COLORS[k], alpha=0.55, label=k)
                 for k in ("LST", "TST", "HST", "FSST", "CS", "UNCERTAIN")],
        loc="lower right", fontsize=6.5, framealpha=0.95, handlelength=1.0, ncol=2)
    ...
    ax_dep.set_title("Depositional\nEnvironment (EOD)",
                     fontsize=10, fontweight="bold", pad=6, color="#006064")
    ...
    for ivl in intervals:
        ...
        if base_c - top_c >= 40:
            ax_dep.text(0.5, (top_c + base_c) / 2,
                        f"{lbl}\n[Rank {rank}]\n{int(t)}–{int(b)} m",
                        ha="center", va="center", fontsize=7,
                        color=fc, fontweight="bold", zorder=5)
        elif base_c - top_c >= 15:
            ax_dep.text(0.5, (top_c + base_c) / 2, lbl,
    ...
    ax_sl.set_title("Sea Level\n(Haq & Ogg 2024)", fontsize=9,
                    fontweight="bold", pad=6, color="#0277BD")
    ...
    if sl_v is not None:
        ...
        ax_sl.fill_betweenx(sl_d, 0, sl_v, where=(sl_v >= 0),
                            color="#90CAF9", alpha=0.82)
        ax_sl.fill_betweenx(sl_d, 0, sl_v, where=(sl_v < 0),
                            color="#FFCC80", alpha=0.82)
    ...
    NN_BG = {
        "NN4": "#FFF3E0", "NN5": "#FFF9C4", "NN6": "#F0F4C3",
        "NN7": "#DCEDC8", "NN8": "#C8E6C9", "NN9": "#B2DFDB",
        "NN10": "#B3E5FC", "NN11": "#BBDEFB",
        "NN11A": "#C5CAE9", "NN11B": "#D1C4E9", "NN11C": "#E8EAF6",
    }
    for ivl in intervals:
        t = float(ivl.top if hasattr(ivl, 'top') else ivl.get("T", 0))
        b = float(ivl.base if hasattr(ivl, 'base') else ivl.get("B", 0))
        ...
        if age and base_c - top_c >= 20:
            ax_age.text(0.5, top_c + 2, f"{age[0]:.1f} Ma",
                        ha="center", va="top", fontsize=7,
                        color="#1A237E", fontweight="bold")
            ax_age.text(0.5, mid_a, zn, ha="center", va="center",
                        fontsize=5.5, color="#37474F", style="italic")
            ax_age.text(0.5, base_c - 2, f"{age[1]:.1f} Ma",
                        ha="center", va="bottom", fontsize=6, color="#546E7A")
        elif age:
            ax_age.text(0.5, mid_a, f"{age[0]:.1f}", ha="center", va="center",
                        fontsize=6, color="#1A237E", fontweight="bold")
...
def generate_correlation_panel(
    ...
    for col_i, well_id in enumerate(well_order):
        ...
        if dep is not None and gr is not None:
            ...
            if len(dep_f) > 6000:
                ...
                dep_f = dep_f[::s]
                gr_f = gr_f[::s]
            _plot_gr_conventional(ax, dep_f, gr_f,
                                  config.gr_min_api, config.gr_max_api, config.gr_cut_api,
                                  config.sand_color, config.shale_color, config.curve_color,
                                  alpha=0.62)
        ...
        ax.tick_params(axis="y", labelsize=7 if col_i == 0 else 0,
                       left=(col_i == 0), length=3)
        ...
        ax.set_title(well_id, fontsize=7.5, fontweight="bold", pad=4,
                     color="#1A237E")
    ...
    fig.suptitle(
        ...
        fontsize=11, fontweight="bold", color="#1A1A2E", y=0.998)

    fig.legend(
        handles=[mpatches.Patch(color=TRACT_COLORS[k], alpha=0.75, label=k)
                 for k in ("LST", "TST", "HST", "FSST", "CS", "UNCERTAIN")],
        loc="lower center", ncol=6, fontsize=7.5, framealpha=0.95,
...
def _parse_zone_age(zone: str, nn_ages: dict):
    ...
    if parts:
        return (min(nn_ages[p][0] for p in parts),
...
def _get_haq_ogg():
    ...
    return np.array([
        [0, 0], [0.12, -120], [0.40, 10], [0.80, -60], [1.20, 20],
        [1.80, -20], [2.50, 30], [3.00, 50], [3.60, 35], [4.20, 45],
        [5.00, -80], [5.40, 60], [5.90, 40], [6.40, 25], [7.00, 35],
        [7.50, 20], [8.00, 45], [8.60, 30], [9.00, 55], [9.50, 40],
        [10.00, 25], [10.50, 35], [11.00, 50], [11.50, 55], [12.00, 65],
        [12.50, 50], [13.00, 30], [13.50, 20], [14.00, 55], [14.50, 70],
        [15.00, 60], [16.00, 50], [17.00, 40], [18.00, 30], [20.00, 35],
        [22.00, 20], [25.00, 15],
    ])
