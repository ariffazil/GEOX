...
def _crop_seismic_panel(arr: np.ndarray) -> tuple:
    ...
    r0 = max(0, r0 - 5); r1 = min(arr.shape[0], r1 + 5)
    c0 = max(0, c0 - 5); c1 = min(arr.shape[1], c1 + 5)
...
HORIZON_COLORS = [
    '#00FF87', '#00D4FF', '#FFE566', '#FF6BD6',
    '#87CEEB', '#FF9F43', '#A8FF78', '#FF8EFF',
]

def _render(raw_arr: np.ndarray, crop_bbox: list,
            attrs: dict, faults: list, horizons: list,
            prov: dict, output_dir: str) -> list:
    ...
    for i, h in enumerate(horizons):
        ...
        ax.plot(pts[:, 0], pts[:, 1], '-', color=col, linewidth=2.2, alpha=0.90,
        ...
        ax.text(lx, ly - 5, f'{h["id"]} ({conf_pct}%)',
                color=col, fontsize=9, fontweight='bold', va='bottom',
                path_effects=[pe.withStroke(linewidth=2, foreground='black')])
    
    # Draw faults
    for f in faults:
        pts = np.array(f["pts"])
        ax.plot(pts[:, 1], pts[:, 0], '-', color=FAULT_COLOR,
                linewidth=2.5, alpha=0.88,
                path_effects=[pe.withStroke(linewidth=4, foreground='black')])
        mid = len(pts) // 2
        conf_pct = int(f["conf_proxy"] * 100)
        ax.text(pts[mid, 1] + 8, pts[mid, 0],
                f'{f["id"]}\n{conf_pct}%',
    ...
    ax.text(0.01, 0.99,
            ...
            transform=ax.transAxes, color='#FFE566', fontsize=8,
            va='top', ha='left',
    ...
    legend_elements = [
        ...
        Line2D([0], [0], color=HORIZON_COLORS[i % len(HORIZON_COLORS)], lw=2.2,
    ...
    # Provenance stamp
    ax.text(0.99, 0.01, prov_short,
            transform=ax.transAxes, color='#555', fontsize=6,
    ...
    fig.suptitle(
        f'GEOX v3.3 — Multi-Attribute Stack\n{prov_short}',
    ...
    for i, h in enumerate(horizons):
        pts = np.array(h["pts"])
        col = HORIZON_COLORS[i % len(HORIZON_COLORS)]
        ax.plot(pts[:, 0], pts[:, 1], '-', color=col, linewidth=2.8, alpha=0.95,
                path_effects=[pe.withStroke(linewidth=4.5, foreground='black')])
        lx = len(pts) // 2
        ly = int(pts[lx, 1])
        ax.text(pts[lx, 0] + 5, ly - 8,
    ...
    for f in faults[:12]:  # cap for readability
        pts = np.array(f["pts"])
        ax.plot(pts[:, 1], pts[:, 0], '-', color='#CC0000',
    ...
    ax.text(0.01, 0.98, "\n".join(note_lines),
            ...
            bbox=dict(boxstyle='round,pad=0.5', facecolor='lightyellow',
                      edgecolor='#CC8800', alpha=0.95))
    
    ax.set_title(
        f"GEOX Physical Reality — Geologist Section\n"
        f"{len(horizons)} horizons  |  {len(faults)} faults  |  {prov['run_tag']}",
...
# ═══════════════════════════════════════════════════════════════════════════
def _build_report(faults: list, horizons: list, prov: dict,
                  gate: dict, crop_bbox: list, outputs: list) -> dict:
    ...
    report = {
        ...
        "faults": [
            {k: v for k, v in f.items() if k != 'pts'}
            | {"polyline_pts": f["pts"][:20]}  # truncated for JSON readability
            for f in faults
        ],
        "fault_polylines_full": {f["id"]: f["pts"] for f in faults},
        "horizons": [
            {k: v for k, v in h.items() if k != 'pts'}
            | {"polyline_sample": h["pts"][::max(1, len(h["pts"]) // 20)]}
            for h in horizons
        ],
        ...
        "epistemic": {
            ...
            "DER_IMAGE_CONTRAST": [
                "AGC amplitude", "Cosine phase", "Coherence",
                "Discontinuity", "Edge magnitude", "Dip chaos", "Fault probability",
            ],
