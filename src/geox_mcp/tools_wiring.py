...
def register_tools_on(mcp):
    ...
    async def _deep_time_unified(
        ...
        if mode in (
            ...
            return {
                "ok": False,
                "error": (
                    f"Unknown mode: {mode}. Valid: dde_reason, tectonic_context, lithology_at_point, age_constraints, query_dde"
                ),
            }
    ...
    async def _model_unified(
        ...
        if mode == "subsurface":
            ...
            # af-fix #2 (scar MEDIUM 0.6): bare-dict error replaced with proper
            # envelope + mode_echo. Previously this returned a plain dict with
            # only "error" key — no isError, no execution_status, no mode echo,
            # no governance_status. Callers saw a hard Python-style error
            # without a structured F2/F4 envelope to route.
            return {
                "tool": "geox_model",
                "mode": mode,
                "mode_echo": mode,
                ...
                "valid_modes": ["subsurface", "geological_generate", "gempy_3d"],
                "error": (f"Unknown mode: {mode}. Valid: subsurface, geological_generate, gempy_3d"),
                "_evidence_postcondition": {
                    "applied": True,
                    "verdict": "DOWNGRADED",
                    "reason": (
                        f"mode={mode!r} is not a recognized geox_model mode; falling through silently would be an F2 violation"
                    ),
