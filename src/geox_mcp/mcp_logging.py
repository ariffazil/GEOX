"""
geox_mcp.mcp_logging — MCP logging utility (FREEZE — SEP-2577)
...
Canonical ops path going forward:
  stderr + structured receipts + HOLD_CANDIDATE (arifOS bridge)

Still valid while deprecated (wire unchanged):
  - Emit ONLY if capabilities.logging declared
  - stderr always (stdout = JSON-RPC only)
  - scrub secrets/PII; rate limit; default client min = warning

Constitutional enforcement stays in arifOS. This module is transport only.
"""
...
if not _stderr.handlers:
    ...
    _h.setFormatter(logging.Formatter("%(asctime)s %(levelname)s [geox.mcp] %(message)s"))
