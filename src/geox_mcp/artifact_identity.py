...
# ── Artifact Record (full metadata envelope) ─────────────────────────────
...
def make_artifact_record(
    *,
    ...
    """Build the full artifact metadata record wrapping the canonical URI.

    The artifact_uri is the transport reference (never used as geological identity).
    domain_id/uwi are the geological identities used by downstream tools.
    """
    artifact_uri = make_artifact_id(
        kind=kind,
    ...
    record: dict[str, Any] = {
        ...
        "kind": kind,
        "canonical_id": canonical_id,
        "domain_id": domain_id or canonical_id.removeprefix("well:"),
        "content_hash": f"sha256:{content_hash.removeprefix('sha256:')}",
