...
HORIZON_COLORS = ["#00FF87", "#00D4FF", "#FFE566", "#FF6BD6", "#87CEEB", "#FF9F43", "#A8FF78", "#FF8EFF"]
ZONE_COLORS = {
...
def render_cognitive_panel(
    ...
    # ── Grid layout ──────────────────────────────────────────────────
    gs = GridSpec(2, 3, figure=fig, width_ratios=[3, 0.8, 1.0], height_ratios=[3, 1], hspace=0.08, wspace=0.06)
    ...
    for pkg in packages:
        ...
        rect = mpatches.Rectangle((0, r0), wc, r1 - r0, facecolor=fc, alpha=alpha, edgecolor="none", zorder=2)
    ...
    for i, h in enumerate(horizons):
        ...
        artifact_flag = (
            ...
            if any(
                "ARTIFACT" in af
                for af in next(
                    (hh.get("artifact_flags", []) for hh in hypotheses.get("horizons", []) if hh["horizon_id"] == h["id"]), []
                )
            )
            else ""
        )
        # Full epistemic block
        hh_data = next((hh for hh in hypotheses.get("horizons", []) if hh["horizon_id"] == h["id"]), {})
    ...
    for f in faults:
        ...
        ax_main.annotate(
            ...
            fontsize=6.2,
            fontweight="bold",
            arrowprops=dict(arrowstyle="->", color=FAULT_COLOR, lw=1.2, connectionstyle="arc3,rad=0.2"),
        ...
        if at_name == "velocity_pullup_sag":
            for cand in at_data.get("candidates", [])[:2]:
                ...
                if h_match:
                    ...
                    ax_main.fill_between(pts[:, 0], pts[:, 1] - 8, pts[:, 1] + 8, alpha=0.25, color="#FFAA00", zorder=4)
                    ax_main.text(
                        wc * 0.5,
                        pts[wc // 2, 1] + 12,
    ...
    # ════════════════════════════════════════════════════════════════
    ax_legend.axis("off")
    ax_legend.set_title("Zone Architecture", color="#aaccff", fontsize=9, fontweight="bold", pad=6)
    ...
    y_pos -= 0.06
    ax_legend.text(
        0.02, y_pos, "Termination Symbols:", transform=ax_legend.transAxes, color="#aaccff", fontsize=7.5, fontweight="bold"
    )
    y_pos -= 0.07
    for ttype, (sym, scol, label) in TERM_SYMBOLS.items():
        ax_legend.text(0.04, y_pos, f"{sym}  {ttype.split('_')[0]}", transform=ax_legend.transAxes, color=scol, fontsize=7.5)
        ax_legend.text(0.04, y_pos - 0.048, f"   {label}", transform=ax_legend.transAxes, color="#778899", fontsize=6)
        y_pos -= 0.11

    # Sequence significance
    y_pos -= 0.04
    ax_legend.text(0.02, y_pos, "▲ = HIGH seq significance", transform=ax_legend.transAxes, color="#FF6BD6", fontsize=7)
    y_pos -= 0.07
    ax_legend.text(0.02, y_pos, "⚠ = artifact flag — test first", transform=ax_legend.transAxes, color="#FFAA00", fontsize=7)

    # ════════════════════════════════════════════════════════════════
    # HYPOTHESIS TABLE — faults and horizons ranked
    # ════════════════════════════════════════════════════════════════
    ax_table.axis("off")
    ax_table.set_title("Ranked Hypotheses\n(Malay Basin prior)", color="#aaccff", fontsize=9, fontweight="bold", pad=6)

    y = 0.97
    # Faults
    ax_table.text(0.02, y, "FAULTS", transform=ax_table.transAxes, color=FAULT_COLOR, fontsize=8, fontweight="bold")
    y -= 0.06
    for fh in hypotheses.get("faults", []):
        ax_table.text(
            0.02,
            y,
            f"{fh['fault_id']}  {fh['dip_class']}  cap={fh['confidence_cap']:.0%}",
        ...
        for rh in fh["hypotheses_ranked"][:3]:
            ...
            bar = mpatches.FancyBboxPatch(
                ...
                facecolor=FAULT_COLOR,
                alpha=0.25 + rh["prior_prob"] * 0.4,
                transform=ax_table.transAxes,
                clip_on=False,
            )
            ax_table.add_patch(bar)
            short = rh["hypothesis"][:28]
            ax_table.text(0.04, y - 0.012, f"#{rh['rank']} {short}", transform=ax_table.transAxes, color="#ffcccc", fontsize=5.8)
            ax_table.text(
                0.87,
                y - 0.012,
                f"P={rh['prior_prob']:.2f}",
                transform=ax_table.transAxes,
                color="#ff9999",
                fontsize=5.8,
                ha="right",
    ...
    # Horizons
    ax_table.text(0.02, y, "HORIZONS", transform=ax_table.transAxes, color="#00FF87", fontsize=8, fontweight="bold")
    y -= 0.06
    for hh in hypotheses.get("horizons", [])[:4]:
        col = HORIZON_COLORS[
            list(h["id"] for h in horizons).index(hh["horizon_id"]) if hh["horizon_id"] in [h["id"] for h in horizons] else 0
        ]
        ...
        for rh in hh["hypotheses_ranked"][:2]:
            ...
            ax_table.text(
                0.04,
                y,
                f"#{rh['rank']} {short}  P={rh['prior_prob']:.2f}",
                transform=ax_table.transAxes,
                color="#aaddcc",
                fontsize=5.8,
            )
        ...
        if hh["artifact_flags"]:
            ax_table.text(
                0.04, y, f"⚠ {hh['artifact_flags'][0][:35]}", transform=ax_table.transAxes, color="#FFAA00", fontsize=5.5
            )
            y -= 0.038
        y -= 0.01
    ...
    # ════════════════════════════════════════════════════════════════
    ax_fp.imshow(fp, cmap="YlOrRd", aspect="auto", vmin=0, vmax=np.percentile(fp, 98), alpha=0.9)
    ax_fp.set_title("Fault Probability (DER_IMAGE_CONTRAST)", color="#ffddaa", fontsize=8, pad=4)
    ...
    ax_fp.text(
        0.02,
        0.97,
        f"Artifact screen: {artifacts['screen_verdict'][:40]}",
    ...
    ax_epist.set_title("Epistemic Ladder (what tier are we on?)", color="#aaccff", fontsize=9, fontweight="bold", pad=6)
    ...
    for label, col, desc, xpos in ladder:
        ...
        ax_epist.text(
            xpos + 0.105, 0.82, label, transform=ax_epist.transAxes, color=col, fontsize=6.5, fontweight="bold", ha="center"
        )
        ax_epist.text(
            xpos + 0.105, 0.50, desc, transform=ax_epist.transAxes, color="#8899aa", fontsize=6, ha="center", va="center"
        )
    ...
    # ── Figure title ─────────────────────────────────────────────────
    fig.suptitle(
        "GEOX Cognitive Interpretation Panel  ·  CV detects → LLM explains → Agent tests → Geologist judges → Governance seals",
        color="#aaccff",
        fontsize=10,
        fontweight="bold",
        y=0.995,
