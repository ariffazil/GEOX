...
class TestLandsatSTACFetcher:
    ...
    async def test_mcp_tool(self):
        ...
        r = await geox_satellite_catalog(
            SatelliteCatalogRequest(minlatitude=4, maxlatitude=6, minlongitude=110, maxlongitude=115)
        )
...
class TestCanonicalRegistry33:
    ...
    def test_surface_vs_internal_split(self):
        from geox_mcp.registry import CANONICAL_PUBLIC_TOOLS, SURFACE_TOOLS, INTERNAL_TOOLS

        expected_total = len(CANONICAL_PUBLIC_TOOLS)
        assert len(SURFACE_TOOLS) + len(INTERNAL_TOOLS) == expected_total
