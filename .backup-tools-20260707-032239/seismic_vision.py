...
async def geox_vision_time_to_depth(
    image_path: str, max_time_ms: float, max_cmp: float, v_rms_anchor: list, execution_mode: str = "deterministic"
) -> dict[str, Any]:
    """
    ...
    result = engine.generate_calibrated_depth_section(
        image_path=image_path, max_time_ms=max_time_ms, max_cmp=max_cmp, v_rms_anchor=v_rms_anchor, output_path=output_target
    )
    ...
    return {
        ...
        "artifact_refs": {"depth_image": output_target},
        "audit_receipt": {"structural_entropy_delta": result["structural_entropy_delta"], "authority": "F2_PHYSICS_GUARD"},
    }
